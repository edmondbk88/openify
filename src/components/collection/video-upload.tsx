...
export function VideoUpload({ value, onChange, brandColor, locale = 'es' }: VideoUploadProps) {
  ...
  const startPreview = useCallback(async () => {
    ...
    try {
      ...
      setError(t.videoPermissionError)
    }
  }, [t])
  ...
  const uploadVideo = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.error || t.videoUploadError)
      }
      ...
      setError(err instanceof Error ? err.message : t.videoUploadError)
      setState('review')
    }
    ...
    <div className="w-full">
      ...
      {state === 'idle' && (
        ...
        >
          ...
          <span className="text-sm font-medium text-gray-600">{t.videoRecord}</span>
        </button>
      )}
      ...
      {state === 'uploading' && (
        <div className="overflow-hidden rounded-xl bg-gray-900">
          <div className="flex aspect-video w-full flex-col items-center justify-center gap-4">
            ...
            <p className="text-sm text-gray-300">{t.videoUploading}</p>
      ...
      {state === 'done' && value && (
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
          ...
          <div className="flex items-center justify-between px-4 py-2">
            <span className="text-xs text-green-600 font-medium">{t.videoDone}</span>
            <button
              type="button"
