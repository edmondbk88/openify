...
export function IndustrySearchEn({ industries }: { industries: IndustryItem[] }) {
    ...
    <>
      ...
      <div className="mb-10">
        ...
        {query.trim() && (
          <p className="mt-3 text-center text-sm text-gray-500">
            {filtered.length} {filtered.length === 1 ? 'industry found' : 'industries found'}
          </p>
        )}
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((industry) => (
          <Link
            key={industry.slug}
            href={`/en/testimonials-for/${industry.slug}`}
          ...
          >
            ...
            <h2 className="mt-4 text-lg font-semibold text-gray-900 group-hover:text-indigo-600">
              Testimonials for {industry.name}
            </h2>
      ...
      {filtered.length === 0 && (
        <div className="py-12 text-center">
          <p className="text-gray-500">No industries found for &quot;{query}&quot;</p>
        </div>
      )}
      ...
      {hasMore && (
        <div className="mt-10 text-center">
          ...
          >
            Show more industries ({filtered.length - visibleCount} remaining)
          </button>
        </div>
      )}
    </>
