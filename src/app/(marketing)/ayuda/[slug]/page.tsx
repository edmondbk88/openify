...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${guide.title} - Centro de Ayuda | Opinafy`,
    description: guide.description,
    alternates: {
      canonical: `https://opinafy.com/ayuda/${guide.slug}`,
      languages: {
        en: `https://opinafy.com/en/help/${guide.slugEn}`,
      },
    },
    openGraph: {
      type: 'article',
      ...
      description: guide.description,
      url: `https://opinafy.com/ayuda/${guide.slug}`,
    },
  }
}

export default async function AyudaGuidePage({ params }: Props) {
  const { slug } = await params
  const guide = helpGuides.find((g) => g.slug === slug)
  ...
  const webPageJsonLd = webPageSchema({
    name: `${guide.title} - Centro de Ayuda | Opinafy`,
    url: `https://opinafy.com/ayuda/${guide.slug}`,
    description: guide.description,
  })
  ...
  const breadcrumbJsonLd = breadcrumbSchema([
    ...
    { name: guide.title, url: `https://opinafy.com/ayuda/${guide.slug}` },
  ])

  // Find prev/next guides
  const idx = helpGuides.findIndex((g) => g.slug === slug)
  const prev = idx > 0 ? helpGuides[idx - 1] : null
  const next = idx < helpGuides.length - 1 ? helpGuides[idx + 1] : null
    ...
    <div className="bg-gradient-to-b from-indigo-50/50 to-white">
      ...
      <div className="mx-auto max-w-3xl px-4 pt-24 sm:px-6 lg:px-8">
        <nav className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <Link href="/ayuda" className="hover:text-indigo-600 transition-colors">Centro de Ayuda</Link>
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
