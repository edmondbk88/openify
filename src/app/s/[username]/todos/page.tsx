...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: { absolute: `Todos los testimonios de ${profile.full_name || username} | Opinafy` },
    description: `Todos los testimonios verificados de ${profile.full_name || username} en Opinafy.`,
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://opinafy.com/s/${username}/todos`,
    },
  }
}
...
export default async function TodosPage({ params }: PageProps) {
  const { username } = await params
  const profile = await getProfileByUsername(username)
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: bgColor, color: textColor, fontFamily }}>
      {/* Header */}
      <header style={{ borderBottom: `1px solid ${dividerColor}` }}>
        <div className={`mx-auto max-w-4xl px-4 py-8 sm:px-6 ${headerAlign}`}>
          <Link
            href={`/s/${username}`}
          ...
          >
            ...
            Volver a {profile.full_name || username}
          </Link>
          <h1
            className="text-2xl font-bold"
            style={{ color: textColor, fontFamily }}
          >
            Todos los testimonios de {profile.full_name || username}
          </h1>
          <p className="mt-1 text-sm" style={{ color: subtextColor }}>
            {totalTestimonials} testimonios verificados
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        {testimonials.length === 0 ? (
          <div className="text-center py-16">
            <p style={{ color: subtextColor }}>Aun no hay testimonios para mostrar.</p>
          </div>
        ) : (
          <>
            {layout === 'timeline' && (
